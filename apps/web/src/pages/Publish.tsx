  ...
  return (
    <div className="h-full overflow-y-auto flex gap-lg p-lg animate-fade-in">
      <div className="flex-1 flex flex-col gap-lg">
        {/* Platform Selection */}
        <div className="aurora-card animate-fade-in" style={{ boxShadow: 'var(--shadow-2)' }}>
          <div className="mb-md">
            <h3 className="text-title-lg font-semibold tracking-tight text-on-surface">选择发布平台</h3>
            <p className="text-body-sm text-on-surface-variant mt-xs">勾选需要同步发布的平台，仅已连接的平台可选</p>
          </div>
            ...
            {platforms.map((platform, index) => {
              ...
              return (
                <button
                  ...
                  className={`flex flex-col items-center p-lg rounded-lg transition-all duration-normal animate-fade-in ${
                    platform.connected
                      ? isSelected
                        ? 'card-hover border-tertiary bg-purple-light cursor-pointer'
                        : 'card-hover cursor-pointer'
                      : 'bg-surface-muted border border-outline opacity-50 cursor-not-allowed'
                  }`}
                  style={{ animationDelay: `${index * 50}ms`, ...(isSelected ? { boxShadow: 'var(--shadow-glow)' } : {}) }}
                >
                  <div
                    className={`w-12 h-12 rounded-full flex items-center justify-center mb-sm transition-all duration-normal ${platform.color} text-on-primary`}
                  >
                    <svg className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor">
                      <path d={platform.icon} />
                    </svg>
                  </div>
                  <span className="text-body-sm font-medium text-on-surface">{platform.name}</span>
                  {platform.connected ? (
                    isSelected ? (
                      <span className="badge-accent mt-xs">已选</span>
                    ) : (
                      <span className="badge mt-xs">已连接</span>
                    )
                  ) : (
                    <span className="text-body-xs text-on-surface-muted mt-xs">未连接</span>
                  )}
                </button>
              );
            })}
        ...
        {/* Publish Settings */}
        <div className="flex-1 aurora-card flex flex-col animate-fade-in" style={{ boxShadow: 'var(--shadow-2)' }}>
          <div className="flex items-center justify-between p-md border-b border-outline">
            <div>
              <h3 className="text-title-lg font-semibold tracking-tight text-on-surface">发布设置</h3>
              <p className="text-body-sm text-on-surface-variant mt-xs">完善视频信息后即可一键发布到所选平台</p>
            </div>
            <button className="btn-primary transition-all duration-normal">一键发布</button>
          </div>
            ...
            <div className="grid grid-cols-2 gap-lg">
                ...
                <div className="flex gap-md">
                  {MOCK_VIDEOS.map((video) => (
                    <div
                      ...
                      className={`flex-shrink-0 w-32 aspect-video rounded-lg overflow-hidden cursor-pointer transition-all duration-normal bg-surface-muted relative ${
                        selectedVideo?.id === video.id
                          ? 'ring-2 ring-tertiary'
                          : 'ring-1 ring-outline hover:ring-outline-variant'
                      }`}
                    >
                      <img
                        src={video.thumbnail}
                      ...
                      />
                      <span className="absolute bottom-xs right-xs text-body-xs rounded-full px-[8px] py-[2px] bg-primary/70 text-on-primary">
                        {video.duration}
                      </span>
                    </div>
                  ))}
              ...
              <div>
                ...
                <input
                  ...
                  className="input-field w-full transition-all duration-normal"
                  placeholder="输入视频标题"
                />
              </div>
              ...
              <div className="col-span-2">
                ...
                <textarea
                  className="input-field w-full h-32 resize-none transition-all duration-normal"
                  placeholder="输入视频描述..."
                />
              </div>
              ...
              <div className="col-span-2">
                ...
                <div className="flex flex-wrap gap-xs items-center">
                  {['#科幻', '#短剧', '#AI创作'].map((tag) => (
                    <span key={tag} className="badge-accent transition-all duration-normal">{tag}</span>
                  ))}
                  <input
                    type="text"
                    className="input-field w-32 transition-all duration-normal"
                    placeholder="添加标签"
                  />
                </div>
              </div>
      ...
      <div className="w-80 animate-fade-in">
        <AIPanel variant="publish" presetAgents={['distributor']} />
      </div>
    </div>
  );
}
